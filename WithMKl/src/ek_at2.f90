Subroutine ek_at(region,isrc,ifld,xy_src,xy_fld,ekat,tsflag)
!...Subroutine to calculate element matrix ekat
!...The only difference with ek_at.f90 is the subroutine eg_ii_tip_tip: fix the bug in ln(2r/(zeta-eita))
!10/19/09: also, correct the case of same crack element for T-stress (1st approach) with integration on whole crack
!11/1/09: the only difference with ek_at1.f90: change eg_adt_reg_reg using 2ND approach
    ...
    REAL(KIND=DBL), INTENT(OUT)	:: ekat(:,:)
    !...add flag of tstress calculation (4/6/09)
    ...
    if (debug) then
      	print*,'ek_at called for elements: ',isrc,ifld
    endif
	...
	if (isrc.eq.ifld) then
        ...
        if ((ELTYPE(elemid(isrc)).eq.CTIP).or.(ELTYPE(elemid(isrc)).eq.CREGULAR)) then
          	!...tip-tip or regular crack-crack: this case ONLY happens for T-stress calculation
            if (tsflag) then
			!not evaluated if tsflag=.false.
              	if (eltype(elemid(isrc)).eq.CTIP) then
                	!...tip-tip
              		call eg_ii_tip_tip(region,elemid(isrc),eg,ng_out,ng_ln,ng_in,xi(:,ng_out),xi_log(:,ng_ln),xi(:,ng_in),&
            				wi(:,ng_out),wi_log(:,ng_ln),wi(:,ng_in),xy_src)
                	!...call eh_reg_tip since we use regular shape functions for outer tip element
                	call eh_reg_tip(elemid(isrc),elemid(isrc),eh,ng_out,ng_in,xi(:,ng_out),xi(:,ng_in),&
            				wi(:,ng_out),wi(:,ng_in),xy_src,xy_src)
                	ekat = eh - eg
                !...Note: this is not yet updated in ek_at.f90 (for tstress1.f90: integration over whole crack)
                elseif (eltype(elemid(isrc)).eq.CREGULAR) then
                	!...regular crack-crack
                    call eg_ii_reg_reg(region,elemid(isrc),eg,ng_out,ng_ln,ng_in,xi(:,ng_out),xi_log(:,ng_ln),xi(:,ng_in),&
            				wi(:,ng_out),wi_log(:,ng_ln),wi(:,ng_in),xy_src)
                    call eh_reg_reg(elemid(isrc),elemid(isrc),eh,ng_out,ng_in,xi(:,ng_out),xi(:,ng_in),&
            				wi(:,ng_out),wi(:,ng_in),xy_src,xy_src)
            		ekat = eh - eg
                endif
            else
            	print*,'ekat is never called for same crack element except T-stress purpose, check element:',elem_sys2user(isrc)
            	stop
            endif
        ...
        elseif ((ELTYPE(elemid(isrc)).eq.BREGULAR).or.(ELTYPE(elemid(isrc)).eq.INTERFE)) then
            ...
            ekat = eh - eg
          	...
          	print*,'ek_at.f90: current version does not support (same element) with elem type: ',elemid(isrc)
            stop
        endif
    elseif (adjacent(isrc,ifld).gt.0) then
        ...
        elseif (elnode(2,isrc).eq.elnode(2,ifld)) then
            ...
            adjacent_type = 4
          	...
          	print*,'ek_at.f90: error on connectivity of elements: ',elem_sys2user(isrc),elem_sys2user(ifld)
            stop
        endif
        if (ELTYPE(elemid(isrc)).eq.CTIP.and.ELTYPE(elemid(ifld)).eq.CTIP) then
          	!...adjacent: tip-tip, this case ONLY happens for T-stress calculation
            if (tsflag) then
			!not evaluated if tsflag=.false.
              	call eg_adt_reg_tip(region,elemid(isrc),elemid(ifld),eg,ng_out,ng_out,xi(:,ng_out),xi(:,ng_out),&
            				wi(:,ng_out),wi(:,ng_out),xy_src,xy_fld,adjacent_type)
            	call eh_reg_tip(elemid(isrc),elemid(ifld),eh,ng_out,ng_out,xi(:,ng_out),xi(:,ng_out),&
            				wi(:,ng_out),wi(:,ng_out),xy_src,xy_fld)
            	ekat = eh - eg
            else
            	print*,'ekat is never called for adjacent tip-tip, check elements:',elem_sys2user(isrc),elem_sys2user(ifld)
            	stop
            endif
			!this ends the if(tsflag) statement
        elseif (ELTYPE(elemid(isrc)).eq.CTIP.and.ELTYPE(elemid(ifld)).ne.CTIP) then
        	!...adjacent tip-reg, this is the case of ea, but ea is the transpose of eat, thus eat still always reg-tip
            if (tsflag) then
			!not evaluated if tsflag is .false.
              	call eg_adt_reg_reg(region,elemid(isrc),elemid(ifld),eg,ng_out,ng_ln,ng_out,xi(:,ng_out),xi_log(:,ng_ln),xi(:,ng_out),&
            				wi(:,ng_out),wi_log(:,ng_ln),wi(:,ng_out),xy_src,xy_fld,adjacent_type)
            	call eh_reg_reg(elemid(isrc),elemid(ifld),eh,ng_out,ng_out,xi(:,ng_out),xi(:,ng_out),&
            				wi(:,ng_out),wi(:,ng_out),xy_src,xy_fld)
            	ekat = eh - eg
            else
            	print*,'ekat is never called for adjacent tip-reg, check elements:',elem_sys2user(isrc),elem_sys2user(ifld)
            	stop
            endif
			!this ends the if(tsflag) statement
		...
		elseif (ELTYPE(elemid(isrc)).ne.CTIP.and.ELTYPE(elemid(ifld)).ne.CTIP) then
      	...
      	!...separated elements
		!lower_half for separated elements,check besides pg.44 in Han's thesis
        if (ELTYPE(elemid(isrc)).eq.CTIP.and.ELTYPE(elemid(ifld)).eq.CTIP) then
          	!...separated: tip-tip
            if (tsflag) then
              	call eg_sed_reg_tip(region,elemid(isrc),elemid(ifld),eg,ng_out,ng_out,xi(:,ng_out),xi(:,ng_out),&
            				wi(:,ng_out),wi(:,ng_out),xy_src,xy_fld)
            	call eh_reg_tip(elemid(isrc),elemid(ifld),eh,ng_out,ng_out,xi(:,ng_out),xi(:,ng_out),&
            				wi(:,ng_out),wi(:,ng_out),xy_src,xy_fld)
                ekat = eh - eg
            else
              	!...the reason for this stop is only ea is called for the case of separated tip-reg, but in this case
                !ea is the transpose of ekat(ifld,iscr), then it goes back to the case ekat of separated reg-tip
            	print*,'ekat is never called for separated tip-tip:',elem_sys2user(isrc),elem_sys2user(ifld)
            	stop
            endif
        elseif (ELTYPE(elemid(isrc)).eq.CTIP.and.ELTYPE(elemid(ifld)).ne.CTIP) then
        	!...separated tip-reg
            if (tsflag) then
              	!...since test function for T-stress calculation used regular shape functions
              	call eg_sed_reg_reg(region,elemid(isrc),elemid(ifld),eg,ng_out,ng_out,xi(:,ng_out),xi(:,ng_out),&
            				wi(:,ng_out),wi(:,ng_out),xy_src,xy_fld)
            	call eh_reg_reg(elemid(isrc),elemid(ifld),eh,ng_out,ng_out,xi(:,ng_out),xi(:,ng_out),&
            				wi(:,ng_out),wi(:,ng_out),xy_src,xy_fld)
            	ekat = eh - eg
            else
              	!...This is temporarily here for the case of interface work, not sure if it is correct
                call eg_sed_tip_reg(region,elemid(isrc),elemid(ifld),eg,ng_out,ng_out,xi(:,ng_out),xi(:,ng_out),&
            				wi(:,ng_out),wi(:,ng_out),xy_src,xy_fld)
            	call eh_tip_reg(elemid(isrc),elemid(ifld),eh,ng_out,ng_out,xi(:,ng_out),xi(:,ng_out),&
            				wi(:,ng_out),wi(:,ng_out),xy_src,xy_fld)
            	ekat = eh - eg
                !delete
                print*,'for interface problem work: ekat called for sed_tip_reg'
            	!print*,'ekat is never called for separated tip-reg:',elem_sys2user(isrc),elem_sys2user(ifld)
            	!stop
            endif
		elseif (ELTYPE(elemid(isrc)).ne.CTIP.and.ELTYPE(elemid(ifld)).eq.CTIP) then
    ...
    CONTAINS
	!--------------------------------------------------
    Subroutine eg_ii_reg_reg(region,idelem,eg,nto,ntoln,nti,xio,xioln,xii,wio,wioln,wii,xlelem)
    IMPLICIT NONE
		...
		do li =1,nti
            ...
            do i=1,neo
              	do j=1,nei
                	do k=1,NDOF
                    	do l=1,NDOF
                       	    f1(l,k,j,i) = pso(i)*dpsi(j)*GI1(l,k)*jacobo
                        enddo
                    enddo
                enddo
            enddo
            ...
            do i=1,neo
              	do j=1,nei
                	do k=1,NDOF
                    	do l=1,NDOF
                       	    f2(l,k,j,i) = pso(i)*dpsi(j)*GI1(l,k)*jacobo
                        enddo
                    enddo
                enddo
            enddo
    ...
    do lo=1,nto
        ...
        do li=1,nti
            ...
            if (r.le.SMALL_NUM) then
              	print*,'eg_ii_reg_reg: r is too small'
                stop
            endif
            !...compute kernels
                	...
                	do k=1,NDOF
                    	do l=1,NDOF
                       		...
                       		val_inner_integral1(l,k,j,i) = val_inner_integral1(l,k,j,i) + &
                            		GI1(l,k)*pso(i)*dpsi(j)*dlog(2.0d0*r/dabs(zeta-eita))*wii(li)
                           	val_inner_integral2(l,k,j,i) = val_inner_integral2(l,k,j,i) + &
                            		G2(l,k)*pso(i)*dpsi(j)*wii(li)
                        enddo
                    enddo
                enddo
            enddo
	...
	End Subroutine eg_ii_reg_reg
    !--------------------------------------------------
	Subroutine eg_ii_tip_tip(region,idelem,eg,nto,ntoln,nti,xio,xioln,xii,wio,wioln,wii,xlelem)
    !...This subroutine is ONLY FOR THE CALCULATION OF T-STRESS
	...
	do lo = 1,ntoln
		...
		do li =1,nti
            ...
            elseif (idelem.eq.CTIP2) then
                ...
                print*,'eg_ii_tip_tip: id of element is out of range'
                stop
            endif
            !...obtain values and derivatives of REGULAR shape functions of
            !(original) outer integration for f(alphapr,betapr)
            ...
            do i=1,neo
              	do j=1,nei
                	do k=1,NDOF
                    	do l=1,NDOF
                    	    if (idelem.eq.CTIP1) then
                        	    f1(l,k,j,i) = (eitapr+1.0d0)*(zetapr+1.0d0)*pso(i)*dpsi(j)*GI1(l,k)*jacobo
                        	else
                        	    f1(l,k,j,i) = (1.0d0-eitapr)*(1.0d0-zetapr)*pso(i)*dpsi(j)*GI1(l,k)*jacobo
                        	endif
                        enddo
                    enddo
                enddo
            ...
            do i=1,neo
              	do j=1,nei
                	do k=1,NDOF
                    	do l=1,NDOF
                    	    if (idelem.eq.CTIP1) then
                        	    f2(l,k,j,i) = (eitapr+1.0d0)*(zetapr+1.0d0)*pso(i)*dpsi(j)*GI1(l,k)*jacobo
                        	else
                        	    f2(l,k,j,i) = (1.0d0-eitapr)*(1.0d0-zetapr)*pso(i)*dpsi(j)*GI1(l,k)*jacobo
                        	endif
                        enddo
                    enddo
                enddo
    ...
    do lo=1,nto
        ...
        do li=1,nti
			...
			!...defensive programming
            if (r.le.SMALL_NUM) then
              	print*,'eg_ii_tip_tip: r is too small'
                stop
            endif
            !...compute kernels
            call Kernel2(NDOF,region,source,field,C2,G2,U2)
                	...
                	do k=1,NDOF
                    	do l=1,NDOF
                        	if (idelem.eq.CTIP1) then
                            	!...not include jacobian of outer element yet
                        		val_inner_integral1(l,k,j,i) = val_inner_integral1(l,k,j,i) + &
                            		GI1(l,k)*pso(i)*dtipi(j)*(eitapr+1.0d0)*(zetapr+1.0d0)*dlog(4.0d0*r/dabs(zetapr-eitapr)/dabs(2.0d0+zetapr+eitapr))*wii(li)
                            	val_inner_integral2(l,k,j,i) = val_inner_integral2(l,k,j,i) + &
                            		G2(l,k)*pso(i)*dtipi(j)*(eitapr+1.0d0)*(zetapr+1.0d0)*wii(li)
                            else
                              	!...not include jacobian of outer element yet
                              	val_inner_integral1(l,k,j,i) = val_inner_integral1(l,k,j,i) + &
                            		GI1(l,k)*pso(i)*dtipi(j)*(1.0d0-eitapr)*(1.0d0-zetapr)*dlog(4.0d0*r/dabs(zetapr-eitapr)/dabs(2.0d0-zetapr-eitapr))*wii(li)
                            	val_inner_integral2(l,k,j,i) = val_inner_integral2(l,k,j,i) + &
                            		G2(l,k)*pso(i)*dtipi(j)*(1.0d0-eitapr)*(1.0d0-zetapr)*wii(li)
                            endif
                        enddo
                    enddo
                enddo
	...
	do lo = 1,nto
        ...
        elseif (idelem.eq.CTIP2) then
        	gamma = xio(lo)
          	...
          	print*,'eg_ii_tip_tip: wrong id of tip element'
            stop
        endif
        val_inner_integral = 0.0d0
		!...loop over inner integral (Gauss integration)
              	...
              	do j=1,nei
                	do k=1,NDOF
                    	do l=1,NDOF
                        	if (idelem.eq.CTIP1) then
                        		val_inner_integral(l,k,j,i) = val_inner_integral(l,k,j,i) + &
                            		GI1(l,k)*pso(i)*dtipi(j)*(1.0d0+eitapr)*(1.0d0+zetapr)*wii(li)*jacobo
                            elseif (idelem.eq.CTIP2) then
                              	val_inner_integral(l,k,j,i) = val_inner_integral(l,k,j,i) + &
                            		GI1(l,k)*pso(i)*dtipi(j)*(1.0d0-eitapr)*(1.0d0-zetapr)*wii(li)*jacobo
                            endif
		...
		do li =1,nti
            ...
            elseif (idelem.eq.CTIP2) then
                ...
                zeta = 1.0d0-0.5d0*(1.0d0-zetapr)*(1.0d0-zetapr)
              	...
              	print*,'eh_ii_tip_tip: id of tip element is out of range'
                stop
            endif
            !...obtain value of shape function and its derivatives of 
            call reg_shape(idelem,eita,pso,dpso)
	...
	End Subroutine eg_ii_tip_tip
    !-----------------------------------------------
	Subroutine eg_adt_reg_tip(region,ido,idi,eg,nto,nti,xio,xii,wio,wii,xlo,xli,adt_type)
	!...Note: actually, this subroutine is EXACTLY the same as eg_adt_tip_tip from T-stress calculation
    !since that one does not stretch the outer (tip) element
    IMPLICIT NONE
	...
	do lo = 1,nto
        ...
        elseif ((adt_type.eq.1).or.(adt_type.eq.3)) then
			...
			gamma = xio(lo)
          	...
          	print*,'error in eg_adt_reg_tip: adt_type out of range'
            stop
        endif
		!...initialize
        val_inner_integral = 0.0d0
		...
		do li =1,nti
            ...
            !...defensive programming
            if (r.le.SMALL_NUM) then
              	print*,'eg_adt_reg_tip: r is too small'
                stop
            endif
            !...calculate val_inner_integral(l,k,j,i)
            do i=1,neo
                	...
                	do k=1,NDOF
                    	do l=1,NDOF
                        	if (idi.eq.CTIP2) then
                        		val_inner_integral(l,k,j,i) = val_inner_integral(l,k,j,i) + &
                            		GI1(l,k)*pso(i)*dtipi(j)*(1.0d0-zetapr)*dlog(r)*jacobo*wii(li)
            ...
            do i=1,neo
              	do j=1,nei
                	do k=1,NDOF
                    	do l=1,NDOF
                        	if (idi.eq.CTIP2) then
                            	!...not include the jacobian of outer element yet
                            	val_inner_integral(l,k,j,i) = val_inner_integral(l,k,j,i) + &
                            		G2(l,k)*pso(i)*dtipi(j)*(1.0d0-zetapr)*wii(li)
                            elseif (idi.eq.CTIP1) then
                              	val_inner_integral(l,k,j,i) = val_inner_integral(l,k,j,i) + &
                            		G2(l,k)*pso(i)*dtipi(j)*(1.0d0+zetapr)*wii(li)
                            else
    ...
    !...element stiffness matrix
    eg = egbar1 + egbar2 + egtwobar
	End Subroutine eg_adt_reg_tip

    !-----------------------------------------------

    Subroutine eg_adt_reg_reg(region,ido,idi,ek,nto,ntoln,nti,xio,xioln,xii,wio,wioln,wii,xlo,xli,adt_type)
	!...11/1/09: use 2ND approach to improve integration of regular adjacent elements
    IMPLICIT NONE
		...
		do li =1,nti
            ...
            elseif ((adt_type.eq.2).or.(adt_type.eq.4)) then
            	beta = gamma - 1.0d0
              	...
              	print*,'eg_adt_reg_reg: wrong adt_type:',adt_type
                stop
            endif
            !...distinguish adjacent types between 2/4 and 1/3
            if ((adt_type.eq.4).or.(adt_type.eq.3)) then
            ...
            do i=1,neo
              	do j=1,nei
                	do k=1,NDOF
                    	do l=1,NDOF
                        	val_inner_integral(l,k,j,i) = val_inner_integral(l,k,j,i) + &
                            GI1(l,k)*pso(i)*dpsi(j)*jacobo*wii(li)
                        enddo
                    enddo
                enddo
            enddo
	...
	do lo = 1,nto
		...
		do li =1,nti
            ...
            !...defensive programming
            if (r.le.SMALL_NUM) then
              	print*,'eg_adt_reg_reg: r is too small'
                stop
            endif
            !...calculate val_inner_integral(l,k,j,i)
            do i=1,neo
                	...
                	do k=1,NDOF
                    	do l=1,NDOF
                        	if (adt_type.eq.1) then
                        		val_inner_integral(l,k,j,i) = val_inner_integral(l,k,j,i) + &
                            		GI1(l,k)*pso(i)*dpsi(j)*dlog(2.0d0*r/(2.0d0-eita+zeta))*wii(li)
                            elseif (adt_type.eq.3) then
                            	val_inner_integral(l,k,j,i) = val_inner_integral(l,k,j,i) + &
                            		GI1(l,k)*pso(i)*dpsi(j)*dlog(2.0d0*r/(2.0d0-eitabar+zeta))*wii(li)
                            elseif (adt_type.eq.2) then
                        		val_inner_integral(l,k,j,i) = val_inner_integral(l,k,j,i) + &
                            		GI1(l,k)*pso(i)*dpsi(j)*dlog(2.0d0*r/(2.0d0-zeta+eita))*wii(li)
                            elseif (adt_type.eq.4) then
                            	val_inner_integral(l,k,j,i) = val_inner_integral(l,k,j,i) + &
                            	GI1(l,k)*pso(i)*dpsi(j)*dlog(2.0d0*r/(2.0d0-zeta+eitabar))*wii(li)
                            endif
                        enddo
                    enddo
                enddo
	...
	End Subroutine eg_adt_reg_reg
!--------------------------------------------------
    Subroutine eg_sed_reg_tip(region,ido,idi,eg,nto,nti,xio,xii,wio,wii,xlo,xli)
	!...Note: this subroutine is EXACTLY the same as eg_sed_tip_tip of T-stress calculation
    !since that one does not stretch out the outer (tip) element
	...
	do lo = 1,nto
		...
		do li =1,nti
            ...
            if (r.lt.SMALL_NUM) then
              	print*,'eg_sed_reg_tip: r is too small for separated elements'
                stop
            endif
            !...compute kernels
            call Kernel2(NDOF,region,source,field,C2,G2,U2)
                	...
                	do k=1,NDOF
                    	do l=1,NDOF
                        	if (idi.eq.CTIP1) then
                            	!...not include the jacobian of outer element yet
                        		val_inner_integral1(l,k,j,i) = val_inner_integral1(l,k,j,i) + &
                            		GI1(l,k)*pso(i)*dtipi(j)*(1.0d0+zetapr)*dlog(r)*wii(li)
                            	val_inner_integral2(l,k,j,i) = val_inner_integral2(l,k,j,i) + &
                            		G2(l,k)*pso(i)*dtipi(j)*(1.0d0+zetapr)*wii(li)
                            elseif (idi.eq.CTIP2) then
                            	val_inner_integral1(l,k,j,i) = val_inner_integral1(l,k,j,i) + &
                            	...
                            	val_inner_integral2(l,k,j,i) = val_inner_integral2(l,k,j,i) + &
                              	...
                              	print*,'eg_sed_reg_tip: id of inner element is out of range'
                                stop
                            endif
                        enddo
                    enddo
    ...
    !...element stiffness matrix
    eg = egbar + egtwobar
	End Subroutine eg_sed_reg_tip
    !-----------------------------------------------
    Subroutine eg_sed_reg_reg(region,ido,idi,eg,nto,nti,xio,xii,wio,wii,xlo,xli)
	...
	do lo = 1,nto
		...
		do li =1,nti
            ...
            !...defensive programming
            if (r.lt.SMALL_NUM) then
              	print*,'eg_sed_reg_reg: r is too small for separated elements'
                stop
            endif
            !...compute kernels
            call Kernel2(NDOF,region,source,field,C2,G2,U2)
              	...
              	do j=1,nei
                	do k=1,NDOF
                    	do l=1,NDOF
                           	!...not include the jacobian of outer element yet
                       		val_inner_integral1(l,k,j,i) = val_inner_integral1(l,k,j,i) + &
                            		GI1(l,k)*pso(i)*dpsi(j)*dlog(r)*wii(li)
                           	val_inner_integral2(l,k,j,i) = val_inner_integral2(l,k,j,i) + &
                            		G2(l,k)*pso(i)*dpsi(j)*wii(li)
                        enddo
                    enddo
    ...
    !...element stiffness matrix
    eg = egbar + egtwobar
	End Subroutine eg_sed_reg_reg
!-----------------------------------------------

	Subroutine eh_reg_tip(ido,idi,eh,nto,nti,xio,xii,wio,wii,xlo,xli)
    ...
    !...Note: this is EXACTLY the same as the eh_tip_tip of T-stress calculation since that one doesn't stretch outer element
    IMPLICIT NONE
    ...
    do lo=1,nto
        ...
        do li=1,nti
            ...
            elseif (idi.eq.CTIP2) then
                zeta = 1.0d0-0.5d0*(1.0d0-zetapr)*(1.0d0-zetapr)
              	...
              	print*,'eh_reg_tip: inner element must be a tip element'
                stop
            endif
            !...shape function & its derivatives at Gauss point
        	call reg_shape(idi,zeta,psi,dpsi)
			...
			!...defensive programming
            if (r.le.SMALL_NUM) then
              	print*,'eh_reg_tip: r is too small'
                stop
            endif
            if (jacobi.le.SMALL_NUM) then
              	print*,'eh_reg_tip: jacobi is too small'
                stop
            endif
                	...
                	do k=1,NDOF
                    	do l=1,NDOF
                        	...
                        	if (idi.eq.CTIP1) then
                            	!...not include jacobian of outer element yet
                        		val_inner_integral(l,k,j,i) = val_inner_integral(l,k,j,i) + &
                            		HI(l,k)*pso(i)*psitip(j)*(zetapr+1.0d0)*jacobi*wii(li)
                            elseif (idi.eq.CTIP2) then
                              	!...not include jacobian of outer element yet
                              	val_inner_integral(l,k,j,i) = val_inner_integral(l,k,j,i) + &
                            		HI(l,k)*pso(i)*psitip(j)*(1.0d0-zetapr)*jacobi*wii(li)
                            else
                              	print*,'eh_tip_tip: inner element must be a tip element'
                                stop
                            endif
                        enddo
                    enddo
        ...
        enddo
    enddo !over loop of outer integral
	End Subroutine eh_reg_tip
!-----------------------------------------------

	Subroutine eh_reg_reg(ido,idi,eh,nto,nti,xio,xii,wio,wii,xlo,xli)
    !...This subroutine is used for both adjacent/separated elements
    !...Note: this is EXACTLY the same as the above eh_tip_reg
    IMPLICIT NONE
    ...
    do lo=1,nto
        ...
        do li=1,nti
			...
			!...defensive programming
            if (r.le.SMALL_NUM) then
              	print*,'eh_reg_reg: r is too small'
                stop
            endif
            if (jacobi.le.SMALL_NUM) then
              	print*,'eh_reg_reg: jacobi is too small'
                stop
            endif
            !...compute normal vector n(xi) = s x e3
            temp(1) = dxdsi(2)
                	...
                	do k=1,NDOF
                    	do l=1,NDOF
                            ...
                            if (k.eq.l) then
                           	...
                           	!...not include jacobian of outer element yet
                       		val_inner_integral(l,k,j,i) = val_inner_integral(l,k,j,i) + &
                            		HI(l,k)*pso(i)*psi(j)*jacobi*wii(li)
                        enddo
                    enddo
                enddo
            enddo
        ...
        enddo
    enddo !over loop of outer integral
	End Subroutine eh_reg_reg
!-----------------------------------------------
!...The following subroutines are temporarily developed for the purpose of interface elements: separated ekat(tip,reg)
    Subroutine eg_sed_tip_reg(region,ido,idi,eg,nto,nti,xio,xii,wio,wii,xlo,xli)
    ...
    INTEGER					:: neo,nei,lo,li
    REAL(KIND=DBL)	:: egbar(3*NDOF,3*NDOF),egtwobar(3*NDOF,3*NDOF)
    REAL(KIND=DBL)	:: eita,zeta,eitapr
    REAL(KIND=DBL)	:: pso(3),dpso(3),psi(3),dpsi(3),tipo(3)
    INTEGER					:: i,j,k,l,ierr
    REAL(KIND=DBL)	:: val_inner_integral1(NDOF,NDOF,3,3),val_inner_integral2(NDOF,NDOF,3,3)
    REAL(KIND=DBL)	:: C2(NDOF,NDOF),G2(NDOF,NDOF),U2(NDOF,NDOF),GI1(NDOF,NDOF)
    REAL(KIND=DBL)	:: source(2),field(2)
	...
	do lo = 1,nto
        ...
        elseif (ido.eq.CTIP2) then
           	...
           	print*,'eg_sed_tip_reg: id of outer element is out of range'
            stop
        endif
        !...obtain value of shape function and its derivatives of 
        call reg_shape(ido,eita,pso,dpso)
                	...
                	do k=1,NDOF
                    	do l=1,NDOF
                        	if (ido.eq.CTIP1) then
                            	!...not include the jacobian of outer element yet
                        		val_inner_integral1(l,k,j,i) = val_inner_integral1(l,k,j,i) + &
                            		GI1(l,k)*tipo(i)*dpsi(j)*(1.0d0+eitapr)*dlog(r)*wii(li)
                            	val_inner_integral2(l,k,j,i) = val_inner_integral2(l,k,j,i) + &
                            		G2(l,k)*tipo(i)*dpsi(j)*(1.0d0+eitapr)*wii(li)
                            elseif (ido.eq.CTIP2) then
                            	val_inner_integral1(l,k,j,i) = val_inner_integral1(l,k,j,i) + &
                            		GI1(l,k)*tipo(i)*dpsi(j)*(1.0d0-eitapr)*dlog(r)*wii(li)
                            	val_inner_integral2(l,k,j,i) = val_inner_integral2(l,k,j,i) + &
                              	...
                              	print*,'eg_sed_tip_reg: id of inner element is out of range'
                                stop
                            endif
                        enddo
                    enddo
    ...
    !...element stiffness matrix
    eg = egbar + egtwobar
	End Subroutine eg_sed_tip_reg
    !-----------------------------------------------

	Subroutine eh_tip_reg(ido,idi,eh,nto,nti,xio,xii,wio,wii,xlo,xli)
    IMPLICIT NONE
    ...
    do lo=1,nto
        ...
        elseif (ido.eq.CTIP2) then
            eita = 1.0d0-0.5d0*(1.0d0-eitapr)*(1.0d0-eitapr)
          	...
          	print*,'eh_tip_reg: outer element must be a tip element'
            stop
        endif
        !...shape function & its derivatives at Gauss point
        call reg_shape(ido,eita,pso,dpso)
                	...
                	do k=1,NDOF
                    	do l=1,NDOF
                        	...
                        	if (ido.eq.CTIP1) then
                            	!...not include jacobian of outer element yet
                        		val_inner_integral(l,k,j,i) = val_inner_integral(l,k,j,i) + &
                            		HI(l,k)*psotip(i)*psi(j)*(eitapr+1.0d0)*jacobi*wii(li)
                            elseif (ido.eq.CTIP2) then
                              	!...not include jacobian of outer element yet
                              	val_inner_integral(l,k,j,i) = val_inner_integral(l,k,j,i) + &
                            		HI(l,k)*psotip(i)*psi(j)*(1.0d0-eitapr)*jacobi*wii(li)
                            else
                              	print*,'eh_tip_reg: outer element must be a tip element'
                                stop
                            endif
        ...
        enddo
    enddo !over loop of outer integral
	End Subroutine eh_tip_reg
End Subroutine ek_at
