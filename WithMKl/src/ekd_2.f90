		...
		do li=1,nti
			...
			call findB2(phi,B2,xlelem,xlelem,pso,dpso,psi,dpsi)
          	        ...
          	        !    do k=1,2
                        ...
                        !    enddo
                        ! enddo
call dgemv('N',2,neo,1.0d0,xlelem,2,pso,1,0.0d0,source,1)
          		 ...
          		 !    do k=1,2
            		 !        field(k) = field(k) + xlelem(k,i)*psi(i)
            	         !    enddo
        	         !enddo
 call dgemv('N',2,nei,1.0d0,xlelem,2,psi,1,0.0d0,field,1)       
                         !...compute r
                         do k=1,2
		...
		do li =1,nti
            ...
            !...position of source point (y)
          	         ...
          	         !   do k=1,2
                         ...
                         !   enddo
                         !enddo
call dgemv('N',2,neo,1.0d0,xlo,2,pso,1,0.0d0,source,1)
                         !...position of field point (xi)
        	         field = 0.0d0
        	         !do i=1,nei
          		 !    do k=1,2
            		 !        field(k) = field(k) + xli(k,i)*psi(i)
            	         !    enddo
        	         !enddo
 call dgemv('N',2,nei,1.0d0,xli,2,psi,1,0.0d0,field,1)       
                         !...compute r
                         do k=1,2
              	             temp(k) = field(k) - source(k)
                         enddo
...
r=dnrm2(2,temp,1)
	...
	REAL(KIND=DBL)	:: field(2),source(2),r,temp(2)
    real*8::dnrm2
    real*8::temp1,temp2,temp3,temp4,temp5,temp6,temp7,temp8,temp9,temp10,temp11,temp12
	!...get number of nodes of outer/inner elements
    neo = NODE(ido)
    nei = NODE(idi)
              	...
              	do j=1,nei
                    ...
                    do k=1,NDOF
                    	do l=1,NDOF
                            ...
                            temp10=E1(l,k)
                            temp11=E2(l,k)
                            val_inner_integral1(l,k,j,i) = val_inner_integral1(l,k,j,i) + (temp8*temp6 + temp10*temp7)*dlog(r)*temp1
                            val_inner_integral2(l,k,j,i) = val_inner_integral2(l,k,j,i) + (temp9*temp6 + temp11*temp7)*temp1
			enddo
                    enddo
                enddo
            enddo
            	...
            	do k=1,NDOF
                	do l=1,NDOF
                    	ekbar(NDOF*(i-1)+k,NDOF*(j-1)+l) = ekbar(NDOF*(i-1)+k,NDOF*(j-1)+l) + &
                        	val_inner_integral1(l,k,j,i)*temp12
                        ek2bar(NDOF*(i-1)+k,NDOF*(j-1)+l) = ek2bar(NDOF*(i-1)+k,NDOF*(j-1)+l) + &
                        	val_inner_integral2(l,k,j,i)*temp12
                    enddo
                enddo
            enddo
        enddo
...
!----------------------
Subroutine Normal_Position(xl,ps,dps,pos,normal,dj)
...
!...shape function and derivative of shape function
    ...
    REAL(KIND=DBL), INTENT(OUT)	:: pos(2),normal(3),dj
...
!...initialising all local variables
    ...
    call dgemv('N',2,3,1.0d0,xl,2,dps,1,0.0d0,dxds,1)
    dj=dnrm2(2,dxds,1)
...
!...this chunk of code is same as that in subroutine nodenormal in prep.f90
    ...
    call dgemv('N',2,3,1.0d0,xl,2,ps,1,0.0d0,pos,1)
	!-----------
...
!...kdelta is simple kronecker delta
   ...
   x1 = -2.0d0*30.0d0*ht**3.0d0*(5.0d0*a + 8.0d0*ht**2.0d0)
   x1 = x1/(225.0d0*ht**2.0d0)

   B1=0.0d0

   do l = 1,2
        do k = 1,2
		     B1(l,k)=phi/(ht**2.0d0)*(w1*temp(l)*temp(k)+x1*kdelta(k,l))
        enddo
   enddo
B1(3,3)=1.0d0
...
!...kdelta is kronecker delta	
   ...
   w2 = -30.0d0*r - 2.0d0*r**3.0d0/(ht**2.0d0) + 26.0d0*dsqrt(a + 4.0d0*ht**2.0d0) + 2.0d0*a*dsqrt(a + 4.0d0*ht**2.0d0)/(ht**2.0d0) + &
                12.0d0*ht**2.0d0*dsqrt(a + 4.0d0*ht**2.0d0)/a - 2.0d0*15.0d0*ht*dlog(2.0d0*ht + dsqrt(a + 4.0d0*ht**2.0d0)) 
			   
   w2 = w2*4.0d0/45.0d0

   !...evaluated w2			   			      

   x2 = -r**5.0d0 - 25.0d0*r**3.0d0*ht**2.0d0 + a**2.0d0*dsqrt(a + 4.0d0*ht**2.0d0) + 23.0d0*a*ht**2.0d0*dsqrt(a + 4.0d0*ht**2.0d0) + &
                31.0d0*ht**4.0d0*dsqrt(a + 4.0d0*ht**2.0d0) 
			   
   x2 = x2*-8.0d0
   
   x2 = x2 + 2.0d0*30.0d0*ht**3.0d0*(5.0d0*a + 8.0d0*ht**2.0d0)*&
               dlog(2.0d0*ht + dsqrt(a + 4.0d0*ht**2.0d0))

   x2 = x2/(225.0d0*ht**2.0d0)
   ...
   do l = 1,2
        do k = 1,2
		     B2(l,k) = phi/(ht**2.0d0)*(w2*temp(l)*temp(k)+x2*kdelta(k,l))
        enddo
   enddo
...
z1 = -2.0d0*3.0d0*ht**3.0d0
z1 = z1*2.0d0/9.0d0

y1 = 0.0d0

temporary1 = 4.0d0*z1*phi/(ht**4.0d0)

temporary2 = 4.0d0*y1*phi/(ht**4.0d0) 
...
z2 = r**3.0d0 + 9.0d0*r*ht**2.0d0 - a*dsqrt(a + 4.0d0*ht**2.0d0) - 7.0d0*ht**2.0d0*dsqrt(a + 4.0d0*ht**2.0d0) + &
     2.0d0*3.0d0*ht**3.0d0*dlog(2.0d0*ht + dsqrt(a + 4.0d0*ht**2.0d0))
z2 = z2*2.0d0/9.0d0

y2 = -r**3.0d0 - 3.0d0*r*ht**2.0d0 + a*dsqrt(a + 4.0d0*ht**2.0d0) + ht**2.0d0*dsqrt(a + 4.0d0*ht**2.0d0)

y2 = y2*2.0d0/(3.0d0*a)

temporary1 = 4.0d0*z2*phi/(ht**4.0d0)

temporary2 = 4.0d0*y2*phi/(ht**4.0d0)
